...
Logique pure (`tests/test_main.py` : validation des scores, `get_client_ip`) + intégration HTTP (`tests/test_api.py` : vraies routes FastAPI via `TestClient`, DB SQLite temporaire par test) :

```bash
pip install -r requirements-dev.txt && pytest   # 25 tests
```
