...
- [x] **Code couleur par rôle, pas par esthétique seule** : le vaisseau du joueur et ses tirs ont leur propre famille de couleurs (cyan/jaune), les ennemis normaux sont rouge, les ennemis plus dangereux ("élite") sont violets, chaque bonus a sa propre couleur distincte. Un joueur peut donc reconnaître "qui/quoi" juste par la couleur, sans lire de texte.
...
- [x] **Le décor (étoiles, planètes, arrière-plan du boss) utilise des teintes désaturées ou réservées** (une plage de vert qu'aucun autre élément du jeu n'utilise, ou du gris "métallique" terne) — l'œil doit pouvoir ignorer le décor sans effort et se concentrer sur ce qui est réellement dangereux/interactif.
